...
def data_treat(data):
    ...
    while(1):
        ...
        if flag_marker:
            ...
            marker = mylist[0]
            data = ')'.join(mylist[1:])
            two_indicators = marker.split('x')
            # print two_indicators
            reference = int(two_indicators[0])
            times = int(two_indicators[1])
            data_string = data[0:reference]
            data_string *= times
    ...
    '''# 使用迭代，但是有4条数据迭代过深得不到结果
def treat_data_part2(data_string):
            ...
            while(1):
                ...
                if not flag_marker:    
                    if data_string[index] == '(':
                        ...
                        pass
                if flag_marker:
                    data_to_treat = data_string[index + 1:]
                    mylist = data_to_treat.split(')')
                    two_indicators = mylist[0].split('x')
                    data_to_treat = ')'.join(mylist[1:])
                    # print data_to_treat
                    reference = int(two_indicators[0])
                    times = int(two_indicators[1])
                    decompass = data_to_treat[0:reference]
                    decompass *= times
                    data_string = data_string[0:index] + decompass + data_to_treat[reference:]
                    # print data_string
                    break
            data_string = treat_data_part2(data_string)
        except:
            return ''
    return data_string         
    '''
    ''' 这个方法是把string展开然后求len(),但是这个string太长了，全部展开内存不够，所以不能使用这个方法
def treat_data_part2_while(data_string):
    if '(' in data_string:
        ok = False
        index = -1
        flag_marker = False
    ...
    '''
# 递归简直碉堡了，效率简直让人高潮！
def decompass_string(data_string):
    if '(' not in data_string:
        ...
        while(1):
            ...
            if flag_marker:
                data_to_treat = data_string[index + 1:]
                mylist = data_to_treat.split(')')
                length_to_skip = len(mylist[0])
                two_indicators = mylist[0].split('x')
                data_to_treat = ')'.join(mylist[1:])
                # print data_to_treat
                reference = int(two_indicators[0])
                times = int(two_indicators[1])
                length += decompass_string(data_to_treat[0:reference])*times
                flag_marker = False
                index += length_to_skip + reference + 1
                # data_string = data_string[0:index] + decompass + data_to_treat[reference:]
        return length         
...
def data_length(list):
    lengths = 0
    # for one in list:
        # length += len(one)  # part1
    total = len(list)
    pbar = ProgressBar().start()
    i = 0
    for one in list:
        if one != '':
            decompass = data_treat(one)
            length = len(decompass)
            if length == 0:
                decompass = data_treat(one)
                length = len(decompass)
            lengths += length
